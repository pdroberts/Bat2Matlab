...
for test_num = tests_2_process
    ...
    elseif strcmp(test.testtype,'twotone')
        testtype = 2;
        if num_traces < 3
            display(['Test ' int2str(test_num) ' does not have standart two-tone test structure. Continuing.']);
            continue;
        end
    ...
    if calculate_tuning_type && length(unique_frequencies) > 1
        ...
        if isempty(thresholdIndex) || unique_frequencies(maxTune) < unique_frequencies(thresholdIndex(1))
            if unique_frequencies(maxTune)-unique_frequencies(minTune) < 8
                ...
                disp('Tuning type: narrow-low')
            else
                tuneClass = 2;
                output_path = [base_output_path prefs.cell_id '_broad-low'];
                disp('Tuning type: broad-low')
            end
        else
...
%                         output_path = [output_path ' ' prefs.colormap_name];
                ...
                title(figure_title);
                if exist('output_path','var') && ~isempty(output_path)
                    PrintFigure([output_path '_spike_count_versus_loudness'],image_format,5,4,600,f);
                    close(f);
                end
            end
...
%                     output_path = [output_path ' ' prefs.colormap_name];
            ...
            title(figure_title);
            if exist('output_path','var') && ~isempty(output_path)
                PrintFigure([output_path '_spike_count_versus_frequency'],image_format,5,4,600,f);
                close(f);
            end
        else
...
%                         output_path = [output_path ' ' prefs.colormap_name];
                ...
                if exist('output_path','var') && ~isempty(output_path)
                    PrintFigure([output_path '_tuning_curve_surface'],image_format,5,4,600,f);
                    close(f);
                end
            end
            if pf(plot_contour)
...
%                         output_path = [output_path ' ' prefs.colormap_name];
                ...
                if exist('output_path','var') && ~isempty(output_path)
                    PrintFigure([output_path '_tuning_curve_contour'],image_format,5,4,600,f);
                    close(f);
                end
            end
            if pf(plot_imagemap)
...
%                 surf(X,Y,test_data','EdgeAlpha',0);
                ...
                if exist('output_path','var') && ~isempty(output_path)
                    PrintFigure([output_path '_tuning_curve_imagemap'],image_format,5,4,600,f);
                    close(f);
                end
            end
        end
    ...
    if pf(plot_histograms)
        ...
        for attenuation = unique_attenuations'
            ...
            if num_freqs == 1
                display(['Only one frequency found while generating ' figure_title]);
%                 f = figure;
...
%                             output_path = [output_path ' ' prefs.colormap_name];
                    ...
                    if exist('output_path','var') && ~isempty(output_path)
                        PrintFigure([hist_output_path '_histogram_surface'],image_format,5,4,600,f);
                        close(f);
                    end
                end
                if pf(plot_contour)
...
%                             output_path = [output_path ' ' prefs.colormap_name];
                    ...
                    if exist('output_path','var') && ~isempty(output_path)
                        PrintFigure([hist_output_path '_histogram_contour'],image_format,5,4,600,f);
                        close(f);
                    end
                end
                if pf(plot_imagemap)
...
%                             output_path = [output_path ' ' prefs.colormap_name];
                    ...
                    if exist('output_path','var') && ~isempty(output_path)
                        PrintFigure([hist_output_path '_histogram_imagemap'],image_format,5,4,600,f);
                        close(f);
                    end
                end
            end
